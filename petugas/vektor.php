                    ...
                    <div class="card-body">
                        <table id="datatablesSimple" data-bs-toggle="tooltip" title="Klik untuk melihat detail data">
                            ...
                            <?php
                                ...
                                // Misalkan variabel session menyimpan id pengguna yang login
                                $idpuskesmas = $_SESSION['idpuskesmas'];

                                // Query untuk mendapatkan data pengendalian vektor yang berhubungan dengan pasien yang memiliki asalfasyankes yang sama dengan idpuskesmas pengguna yang login, menggunakan DISTINCT
                                $ambildatavektor = mysqli_query($conn, "
                                    SELECT DISTINCT v.*, p.namapasien 
                                    FROM pengendalianvektor v 
                                    JOIN pasien p ON p.id = v.id
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var idpuskesmas = <?php echo json_encode($_SESSION['idpuskesmas']); ?>; // Mengambil idpuskesmas dari sesi

    function loadPatients() {
        var xhr = new XMLHttpRequest();
        xhr.open('GET', 'get_patients.php?idpuskesmas=' + idpuskesmas, true); // Mengirim idpuskesmas ke server
        xhr.onload = function() {
            if (this.status === 200) {
